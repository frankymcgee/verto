...
# we use our own creator (below) rather than HRMS one so we can set custom_project
...
@frappe.whitelist()
def get_events(month_start: str, month_end: str,
               employee_filters: dict[str, str], shift_filters: dict[str, str]) -> dict[str, list[dict]]:
    ...
    for event in [holidays, leaves, shifts]:
        for key, value in event.items():
            events.setdefault(key, []).extend(value)
    return events


@frappe.whitelist()
def get_schedule_from_assignment(shift_schedule_assignment: str):
    shift_schedule = frappe.db.get_value("Shift Schedule Assignment",
                                         shift_schedule_assignment, "shift_schedule")
    frequency = frappe.db.get_value("Shift Schedule", shift_schedule, "frequency")
    repeat_on_days = frappe.get_all("Assignment Rule Day",
                                    filters={"parent": shift_schedule}, pluck="day")
...
def create_shift_schedule_assignment(
    ...
    """Create Shift Schedule Assignment and immediately generate up to 90 days of shifts."""
    ...
    assignment = frappe.get_doc({
        ...
        "custom_project": custom_project,   # <-- carry project on the schedule
        "enabled": 0 if end_date else 1,
    }).insert()

    if not end_date or date_diff(end_date, start_date) <= 90:
        return assignment.create_shifts(start_date, end_date)

    frappe.enqueue(assignment.create_shifts, timeout=4500,
...
def delete_shift_schedule_assignment(shift_schedule_assignment: str) -> None:
    for sa in frappe.get_all("Shift Assignment",
        ...
        if doc.docstatus == 1:
            doc.cancel()
        frappe.delete_doc("Shift Assignment", sa)
    frappe.delete_doc("Shift Schedule Assignment", shift_schedule_assignment)


@frappe.whitelist()
def swap_shift(src_shift: str, src_date: str,
               tgt_employee: str, tgt_date: str, tgt_shift: str | None) -> None:
    """Swap or move a shift, preserving project + location."""
    ...
    # move source -> target
    insert_shift(
        employee=tgt_employee,
        company=tgt_company,
        shift_type=src_shift_doc.shift_type,
        start_date=tgt_date,
        end_date=tgt_date,
        status=src_shift_doc.status,
        shift_location=src_shift_doc.shift_location,
        custom_project=getattr(src_shift_doc, "custom_project", None),
    )

    if tgt_shift:
        # move target -> source
        insert_shift(
            employee=src_shift_doc.employee,
            company=src_shift_doc.company,
            shift_type=tgt_shift_doc.shift_type,
            start_date=src_date,
            end_date=src_date,
            status=tgt_shift_doc.status,
            shift_location=tgt_shift_doc.shift_location,
            custom_project=getattr(tgt_shift_doc, "custom_project", None),
        )
...
def break_shift(assignment: str | "ShiftAssignment", date: str) -> None:
    ...
    custom_project = getattr(assignment, "custom_project", None)
...
def insert_shift(
    ...
    """Insert a shift, merging adjacent identical segments; carry project where provided."""
    filters = {
    ...
    next_shift = frappe.db.exists(dict({"start_date": add_days(end_date, 1)}, **filters)) if end_date else None
...
def get_leaves(month_start: str, month_end: str, employee_filters: dict[str, str]) -> dict[str, list[dict]]:
    ...
    q = (
        ...
        .left_join(Employee).on(LeaveApplication.employee == Employee.name)
        .where(
    ...
    for f in employee_filters:
        q = q.where(Employee[f] == employee_filters[f])

    return group_by_employee(q.run(as_dict=True))


def get_shifts(month_start: str, month_end: str,
               employee_filters: dict[str, str], shift_filters: dict[str, str]) -> dict[str, list[dict]]:
    ...
    q = (
        frappe.qb.select(
            ...
            ShiftAssignment.custom_project_name,  # expose for UI
            ShiftType.start_time,
            ShiftType.end_time,
            ShiftType.color,
        )
        .from_(ShiftAssignment)
        .left_join(ShiftType).on(ShiftAssignment.shift_type == ShiftType.name)
        .left_join(Employee).on(ShiftAssignment.employee == Employee.name)
        .where(
    ...
    for f in shift_filters:
        q = q.where(ShiftAssignment[f] == shift_filters[f])
...
def group_by_employee(events: list[dict]) -> dict[str, list[dict]]:
    out = {}
    for e in events:
        out.setdefault(e["employee"], []).append({k: v for k, v in e.items() if k != "employee"})
    return out
