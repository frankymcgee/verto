...
frappe.ui.form.ControlGeolocation = class ControlGeolocation extends frappe.ui.form.ControlData {
	...
	bind_leaflet_map() {
		...
		this.railLayer = L.tileLayer(
			frappe.utils.map_defaults.tiles.rail_tail.url,
			frappe.utils.map_defaults.tiles.rail_tail.options
		);
				...
				Object.values(this.map._layers).forEach((layer) => {
					if (
						...
						(layer._url === frappe.utils.map_defaults.tiles.labels_tail.url ||
							...
							layer._url === frappe.utils.map_defaults.tiles.rail_tail.url)
					) {
