...
@frappe.whitelist()
def verto_get_coords(doctype, filters, type):
	"""Get a geojson dict representing a doctype."""
	filters_sql = verto_get_coords_conditions(doctype, filters)[4:]

	coords = None
	if type == "location_field":
		coords = verto_return_location(doctype, filters_sql)
	elif type == "coordinates":
		coords = verto_return_coordinates(doctype, filters_sql)

	return verto_convert_to_geojson(type, coords)


def verto_convert_to_geojson(type, coords):
	"""Convert GPS coordinates to geoJSON string."""
	geojson = {"type": "FeatureCollection", "features": None}

	if type == "location_field":
		geojson["features"] = verto_merge_location_features_in_one(coords)
	elif type == "coordinates":
		geojson["features"] = verto_create_gps_markers(coords)

	return geojson


def verto_merge_location_features_in_one(coords):
...
def verto_create_gps_markers(coords):
...
def verto_return_location(doctype, filters_sql):
...
def verto_get_coords_conditions(doctype, filters=None):
