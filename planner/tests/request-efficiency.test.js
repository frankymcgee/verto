...
const mocks = vi.hoisted(() => ({ createResource: vi.fn(() => ({ data: null })), frappeRequest: vi.fn(async () => ({})) }))
...
describe('Planner request efficiency', () => {
  it('loads settings once for navbar, apps and view preferences', () => {
    ...
    expect(mocks.createResource.mock.calls[0][0].params.doctype).toBe('Verto Mobile Settings')
  })
  it('snapshots queued filters and respects cancellation', async () => {
    ...
    await request
    expect(mocks.frappeRequest.mock.calls.at(-1)[0].params.employee_filters.company).toBe('original')
