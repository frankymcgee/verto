<template>
  ...
  >
    <div
      class="planner-shift-assignment-dialog-body overflow-visible px-6 py-5"
    >
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div class="min-w-0">
          <Combobox
            ...
            :disabled="!!props.shiftAssignmentName"
            :options="[
              { label: `Select Shift Location`, value: '' },
              ...shiftLocationOptions.map((location) => ({
                label: `${location}`,
                value: location,
              })),
            ]"
        ...
        <FormControl type="textarea" label="Note" v-model="form.note" />
        ...
        >
          ...
          <label
            ...
            class="flex items-start gap-2 rounded-4 border bg-gray-50 px-3 py-2 text-sm text-gray-700"
          >
            <Checkbox v-model="includeFlyInFlyOut" />
            <span>
              <span class="block font-medium text-gray-800"
                >Add fly in / fly out</span
              >
              <span class="block text-xs text-gray-500">
                Creates FI on the first day and FO on the last day of each
                swing.
              </span>
            </span>
          </label>
      ...
      >
        ...
        <div
          :class="
            scheduleType === 'Rolling Day/Night Roster'
              ? 'grid grid-cols-3 gap-6'
              : 'grid grid-cols-1 sm:grid-cols-2 gap-6'
          "
        >
          <div v-if="scheduleType === 'Repeat On Days'" class="min-w-0">
            <Select
              label="Frequency"
              v-model="frequency"
              :disabled="!!props.shiftAssignmentName"
          ...
          >
            <div
              ...
              :key="index"
              class="rounded-4 border bg-white p-3"
            >
              <div class="mb-3 text-sm-medium text-gray-700">
                Swing {{ index + 1 }}
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <FormControl
          ...
          <div v-if="scheduleType === 'Repeat On Days'" class="space-y-1.5">
            ...
            <div
              class="border rounded-4 grid grid-flow-col h-7 justify-stretch overflow-clip"
            >
              <Checkbox v-for="(_, day) of repeatOnDays" :key="day" :label="day.substring(0, 3)" v-model="repeatOnDays[day]" :disabled="!!props.shiftAssignmentName" class="px-2" />
            </div>
          </div>
          ...
          <div
            ...
            :class="
              scheduleType === 'Rolling Day/Night Roster'
                ...
                : 'col-span-2'
            "
            class="rounded-4 border bg-gray-50 px-3 py-2 text-xs text-gray-600"
          >
            <template
              v-if="scheduleType === 'Rolling Roster' && includeFlyInFlyOut"
            >
              Rolling roster will create each swing using <b>FI</b> on the first
              day, the selected Shift Type for the days in between, and
              <b>FO</b> on the final day.
            </template>
            <template v-else-if="scheduleType === 'Rolling Roster'">
              Rolling roster will create each on-site swing using the selected
              Shift Type only. FI and FO shifts will not be created.
            </template>
            <template
              v-else-if="
                scheduleType === 'Rolling Day/Night Roster' &&
            ...
            >
              ...
              then <b>DS</b>, then <b>NS</b>, and <b>FO</b> on the final day of
              each swing.
            </template>
            <template v-else-if="scheduleType === 'Rolling Day/Night Roster'">
              Rolling Day/Night roster will create each swing using
              <b>DS</b> for the day-shift block and <b>NS</b> for the
              night-shift block. FI and FO shifts will not be created.
            </template>
            <template v-else-if="includeFlyInFlyOut">
              Dynamic rolling roster will cycle through each configured swing
              pattern. Each swing will create <b>FI</b> on the first day, the
              selected Shift Type for the days in between, and <b>FO</b> on the
              final day.
            </template>
            <template v-else>
              Dynamic rolling roster will cycle through each configured swing
              pattern and create each on-site day using the selected Shift Type
              only. FI and FO shifts will not be created.
            </template>
          </div>
        </div>
      </div>
        ...
        <div class="flex space-x-3 justify-end">
          ...
          <Dropdown
            ...
            :options="
              actions.map((option) => ({
                label: option.label,
                onClick: () => runDeleteOption(option),
              }))
            "
            :button="{
              label: 'Delete',
  ...
  <Dialog
    :open="dialogOpen && showDeleteDialog"
  ...
  >
    ...
    <template #actions
      ><div
        class="flex justify-end gap-2 border-t border-gray-200 bg-gray-50 px-5 py-4"
      >
        <Button variant="subtle" @click="showDeleteDialog = false"
          >Cancel</Button
        >
...
type ScheduleType =
  | "Repeat On Days"
  | "Rolling Roster"
  | "Rolling Day/Night Roster"
  | "Dynamic Rolling Roster";
...
const rollingDayNightRoster = reactive({
  days_on_site_ds: 8,
  days_on_site_ns: 8,
  days_off_site: 12,
});
...
const visibleDynamicRollingSwings = computed(() => {
  const count = Math.max(
    1,
    Math.min(20, Number(dynamicRollingRoster.swing_count) || 1),
  );
...
// --- Utils
const getId = (val: Selectish) =>
  val && typeof val === "object" ? (val as any).value : (val as string) || "";
...
watch(
  ...
  (val) => {
    ...
    if (props.shiftAssignmentName) {
      ...
      dynamicRollingRoster.swings.splice(
        0,
        dynamicRollingRoster.swings.length,
        { days_on_site: 8, days_off_site: 6 },
        { days_on_site: 4, days_off_site: 3 },
      );
...
const createShiftAssigment = () => {
  ...
  if (
    showShiftScheduleSettings.value &&
    (Object.values(repeatOnDays).some((day) => !day) ||
      frequency.value !== "Every Week")
  ) {
...
const shiftSchedule = createResource({
  ...
  makeParams: () => ({
    shift_schedule_assignment: form.shift_schedule_assignment,
  }),
  onSuccess: (d: { frequency: string; repeat_on_days: string[] }) => {
    frequency.value = d.frequency;
    for (const day in repeatOnDays) {
      repeatOnDays[day as keyof typeof repeatOnDays] =
        d.repeat_on_days.includes(day);
    }
  },
  onError(error: { messages: string[] }) {
    raiseToast("error", error.messages[0]);
...
const projectOptions = computed(() => {
  ...
  if (
    defaults?.custom_project &&
    !options.some(
      (option: { value: string }) => option.value === defaults.custom_project,
    )
  ) {
    options.unshift({
      value: defaults.custom_project,
      label: defaults.project_name || defaults.custom_project,
    });
  }
  return options;
});
  ...
  makeParams() {
    return {
      ...
      repeat_on_days: Object.keys(repeatOnDays).filter(
        (d) => repeatOnDays[d as keyof typeof repeatOnDays],
      ),
