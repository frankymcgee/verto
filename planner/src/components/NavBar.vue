<template>
  <header
    class="planner-navbar sticky top-0 z-40 h-14 border-b bg-white/95 px-4 shadow-sm backdrop-blur md:px-8"
  >
    <div class="flex h-full items-center gap-3">
      <a
        href="/planner"
        class="planner-brand group flex min-w-0 items-center gap-3"
      >
        <div
          class="planner-logo flex h-9 w-9 shrink-0 items-center justify-center rounded-7"
        >
          <img
            v-if="plannerIcon"
        ...
        <div class="min-w-0 leading-tight">
          <div
            class="truncate text-sm-semibold text-gray-900 group-hover:text-gray-700"
          >
            {{ plannerAppName }}
          </div>
        ...
        >
          ...
          >
            ...
            <div class="hidden max-w-[160px] leading-tight md:block">
              ...
              <div class="truncate text-[11px] text-gray-500">Account</div>
            </div>
...
const logout = createResource({
  ...
  onError(error: { messages?: string[]; message?: string }) {
    raiseToast(
      "error",
      error.messages?.[0] || error.message || "Unable to log out",
    );
  },
});
</script>
