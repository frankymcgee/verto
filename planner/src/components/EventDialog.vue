        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <template v-for="field in visibleFields" :key="field.key">
            ...
            >
              ...
              >
                {{ field.fieldtype }} fields are managed from the full Event
                form in Desk.
              </div>
...
import { Checkbox, Textarea, TextInput } from "frappe-ui";
...
const eventMeta = createResource({
  ...
  onError(error: { messages?: string[]; message?: string }) {
    raiseToast(
      "error",
      error?.messages?.[0] || error?.message || "Failed to load Event fields",
    );
  },
});
...
const createEvent = createResource({
  ...
  onSuccess(data: { name?: string; subject?: string }) {
    raiseToast(
      "success",
      `Event created${data?.subject ? `: ${data.subject}` : ""}`,
    );
    emit("fetchEvents");
    closeDialog();
  },
  onError(error: { messages?: string[]; message?: string }) {
    raiseToast(
      "error",
      error?.messages?.[0] || error?.message || "Failed to create Event",
    );
  },
});
...
const fields = computed<EventField[]>(() => {
  const rawFields =
    (eventMeta.data as EventMetaResponse | undefined)?.fields || [];
...
function initialiseForm(inputFields: EventField[]) {
  ...
  if ("event_category" in form && !form.event_category)
    form.event_category = "Event";
  if ("event_type" in form && !form.event_type) form.event_type = "Public";
  if ("status" in form && !form.status) form.status = "Open";
  if ("send_reminder" in form && form.send_reminder === undefined)
    form.send_reminder = true;
  if ("all_day" in form && form.all_day === undefined) form.all_day = false;
  if ("starts_on" in form && !form.starts_on)
    form.starts_on = now.format("YYYY-MM-DDTHH:mm");
  if ("ends_on" in form && !form.ends_on)
    form.ends_on = now.add(1, "hour").format("YYYY-MM-DDTHH:mm");
}
